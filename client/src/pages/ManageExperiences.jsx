...
const ManageExperiences = () => {
  ...
  // Function to fetch company Experience Applications data
  ...
  const changeJobVisibility = async (id) => {
    try {
      const { data } = await axios.post(backendUrl + '/api/company/change-visibility-experience',
        { id },
        {
          headers: { token: companyToken }
        })
  ...
  return jobs ? jobs.length === 0 ? (
    ...
    <div className='container p-4 max-w-5xl'>
      ...
      <div className='mt-4 flex justify-end'>
        <button onClick={() => navigate('/dashboard/add-experience')} className='bg-black text-white py-2 px-4 rounded'>Add new experience</button>
      </div>
    </div>
  ) : <Loading />
}
